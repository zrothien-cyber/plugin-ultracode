...
const { assertClaudeWorkflowSupported, cacheKey, extractClaudeWorkflowMeta, prepareClaudeWorkflowSource, sourceHash } = require("./claude-workflow-compat");
...
function cacheableWorkerMap(record) {
    ...
    for (const worker of (record && record.workers) || []) {
        if (!worker || worker.status !== "completed" || !worker.cache_key)
            continue;
...
function buildScope(ctx, input, hooks = {}) {
    ...
    function normalizeWorkerOpts(opts = {}) {
        ...
        if (normalized.agentType && !normalized.label)
            normalized.label = String(normalized.agentType);
        if (normalized.name && !normalized.label)
            normalized.label = String(normalized.name);
        return normalized;
    }
    ...
    async function pipeline(items, ...stages) {
        ...
        if (!currentPhase && !currentAutoPhase)
            currentAutoPhase = label;
        try {
    ...
    async function fanout(taskOrSpecs, opts = {}) {
        if (Array.isArray(taskOrSpecs)) {
            return parallel(taskOrSpecs.map((spec) => () => {
        ...
        return parallel(engine.selectRoles(count).map((role) => () => agent(engine.workerPrompt({ task, workflow: pseudoWorkflow, worker: role, sandbox }), {
            label: role.title,
    ...
    async function dag(steps) {
        const defaults = {
            ...
            timeout_ms: input.timeout_ms === undefined || input.timeout_ms === null
                ? undefined
                : Math.max(1000, Math.floor(Number(input.timeout_ms))),
            executor: "cold"
        };
        if (typeof input.model === "string" && input.model.trim())
            defaults.model = input.model.trim();
...
async function runScript(input = {}) {
    ...
    if (claudeCompat)
        assertClaudeWorkflowSupported(source);
    ...
    ctx = engine.createContext({
        ...
        onEvent: typeof input.on_event === "function"
            ? (event) => {
                try {
        ...
        onWorkerRecord: (result, meta) => {
            if (!record)
                return;
            const index = meta && meta.id && workersById.has(meta.id) ? workersById.get(meta.id) : record.workers.length;
