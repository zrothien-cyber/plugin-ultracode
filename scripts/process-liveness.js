...
function parseWindowsCreationDate(value) {
  ...
  if (!offset) return new Date(utcMs).toISOString();
...
function normalizeStartTime(value) {
  ...
  if (parsedWindows) return parsedWindows;
...
function lookupWindowsProcessIdentity(pid) {
  ...
  try {
    ...
    if (error && error.status === 3) return { exists: false, pid, platform: "win32" };
...
function lookupPosixProcessIdentity(pid, platform) {
  ...
  if (!trimmed) return { exists: false, pid, platform };
...
function lookupProcessIdentity(pid, platform = process.platform) {
  if (!Number.isInteger(pid) || pid <= 0) return { exists: false, pid, platform };
  if (platform === "win32") return lookupWindowsProcessIdentity(pid);
