...
module.exports = function createPipeline(foundation, execution, workflows) {
    const { DEFAULT_TIMEOUT_MS, VALID_SANDBOXES, VALID_EFFORTS, WORKER_SCHEMA, assertNonEmptyString, resolveModel, resolveReasoningEffort, defaultCodexBin, codexHome, statePathFor, writeJson, createContext, sumUsageFromWorkers, emitEvent, log, firstDefined, resolveBool, resolveTransport } = foundation;
    ...
    function renderTemplate(str, scope) {
        ...
        const out = str.replace(/\{\{\s*([^}]*?)\s*\}\}/g, (match, rawExpr) => {
            ...
            if (expr === "round") {
                if (scope && scope.round !== undefined)
                    return String(scope.round);
                throw new Error(`renderTemplate: {{round}} used outside a loop step.`);
            }
            if (expr === "seen" || expr === "seen_json") {
                if (!scope || !Array.isArray(scope.seen))
                    throw new Error(`renderTemplate: {{${expr}}} used outside a stateful loop step.`);
                return expr === "seen_json" ? JSON.stringify(scope.seen) : (scope.seen.length ? scope.seen.join("\n") : "(none)");
            }
            if (expr === "consecutive_dry") {
                if (scope && scope.consecutive_dry !== undefined)
                    return String(scope.consecutive_dry);
            ...
            if (expr === "item" || expr.startsWith("item.")) {
                ...
                if (rendered === undefined)
                    throw new Error(`renderTemplate: unresolved token {{${expr}}}.`);
                return rendered;
            }
    ...
    function compileSteps(steps, defaults) {
        ...
        const compiled = steps.map((raw, index) => {
            ...
            const schema = raw.schema === null ? null : raw.schema && typeof raw.schema === "object" ? raw.schema : WORKER_SCHEMA;
        ...
        for (const step of compiled) {
            ...
            if (step.kind === "verify" && !step.depends_on.includes(step.findings_from)) {
                throw new Error(`steps "${step.id}" findings_from "${step.findings_from}" must be listed in its depends_on.`);
            }
            ...
            for (const ref of refs) {
                ...
                if (!depSet.has(ref)) {
                    throw new Error(`steps "${step.id}" template references "${ref}" which is not in its depends_on.`);
                }
            }
        }
    ...
    async function executeStep(step, results, ctx, codexBin, codexHomeValue, retryWorker, transportOpts) {
        ...
        if (step.kind === "parallel") {
            ...
            const records = [];
            const thunks = items.map((item, i) => () => spawnWorker(renderTemplate(step.prompt, { ...baseScope, item }), {
                ...common,
                schema: step.schema,
                label: `${step.label}#${i}`,
                isolation: step.isolation,
        ...
        if (step.kind === "verify") {
            ...
            if (!Array.isArray(findings))
                findings = findings === undefined || findings === null ? [] : [findings];
        ...
        if (step.kind === "loop") {
            const collected = await loopUntilDry((round, _ctx, state) => renderTemplate(step.prompt, {
                ...baseScope,
                round,
                seen: state.seenList || [],
    ...
    function stepRecordFromExecution(step, execution, durationMs) {
        ...
        const allCancelled = execution.workerResults.length > 0 && execution.workerResults.every((r) => r && r.status === "cancelled");
        return {
            index: step.index,
            id: step.id,
            step_id: step.id,
    ...
    async function runDagOnCtx(compiled, ctx, opts = {}) {
        ...
        for (const step of compiled) {
            ...
            const promise = Promise.all(depPromises).then(async () => {
                ...
                if (typeof onStepRecord === "function")
    ...
    async function runPipelineSpec(input = {}) {
        ...
        const defaults = {
            ...
            executor: input.executor === "resume" || input.executor === "fork" || input.executor === "cold"
                ? input.executor
                : "cold"
        };
