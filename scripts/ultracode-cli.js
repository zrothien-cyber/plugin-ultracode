...
function parseArgs(argv) {
    ...
    for (; index < rest.length; index += 1) {
        ...
        if (!arg.startsWith("--")) {
            if (!options._positionals)
                options._positionals = [];
...
async function workflowCommand(options) {
    ...
    if (action === "save") {
        ...
        if (!source && options.source_path)
            source = await fs.readFile(options.source_path, "utf8");
        if (!source && (options.workflow_id || options.state_path || options.from_workflow_id)) {
            ...
            if (!record || record.status === "missing")
                throw new Error("No workflow record found to save.");
            const scriptPath = record.script_path || record.source_path;
            if (!scriptPath)
                throw new Error("Workflow record does not include a script_path or source_path.");
    ...
    throw new Error(`Unknown workflow command: ${action} (expected list|show|save|update|delete). ` +
        `To RUN a saved workflow use \`ultracode @${target || "<name>"}\` or \`--workflow <name>\`.`);
}
function autoUpdateDisabled(options) {
    if (options.no_auto_update === true)
        return true;
    if (parseBool(process.env.ULTRACODE_NO_AUTO_UPDATE) === true)
        return true;
    // ULTRACODE_AUTO_UPDATE=0/false is also an explicit opt-out.
    const legacy = process.env.ULTRACODE_AUTO_UPDATE;
    if (legacy !== undefined && parseBool(legacy) === false)
        return true;
    return false;
}
...
function runJsonInput(parsed, options) {
    ...
    if (hasIds && !steps.every(withId)) {
        throw new Error("JSON input mixes a pipeline DAG (every object needs an `id`) with workers_spec entries (no `id`). " +
...
async function runExec(options) {
    ...
    throw new Error("Nothing to run. Provide an input — a task sentence, a script path or --source, a steps JSON or --steps, " +
