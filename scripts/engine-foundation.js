...
function createFoundation() {
    ...
    const WORKER_ROLES = [
        {
            id: "context-scout",
            title: "Context Scout",
            focus: "Map the relevant files, entry points, existing patterns, and constraints. Avoid implementation unless asked."
        },
        {
            id: "implementation-planner",
            title: "Implementation Planner",
            focus: "Propose the smallest coherent implementation path that fits the existing codebase and avoids unnecessary infrastructure."
        },
        {
            id: "risk-reviewer",
            title: "Risk Reviewer",
            focus: "Look for regressions, hidden coupling, missing tests, unsafe assumptions, and behavioral edge cases."
        },
        {
            id: "test-strategist",
            title: "Test Strategist",
            focus: "Identify the most meaningful verification commands, fixtures, and focused tests for the task."
        },
        {
            id: "api-contract-reviewer",
            title: "API Contract Reviewer",
            focus: "Check schemas, public contracts, tool interfaces, and compatibility boundaries."
        },
        {
            id: "cleanup-reviewer",
            title: "Cleanup Reviewer",
            focus: "Find stale paths, redundant abstractions, deprecated code, and opportunities to keep the change lean."
        },
        {
            id: "docs-operator",
            title: "Docs Operator",
            focus: "Identify only durable documentation or instruction updates that are genuinely required."
        },
        {
            id: "final-verifier",
            title: "Final Verifier",
            focus: "Review the proposed path as if signing off on the work. Be concrete about remaining proof needed."
        }
    ];
    const WORKER_SCHEMA = {
        type: "object",
    ...
    function normalizeOptions(input = {}) {
        ...
        return {
            ...
            codex_bin: typeof input.codex_bin === "string" && input.codex_bin.trim()
                ? input.codex_bin.trim()
                : defaultCodexBin(),
            codex_home: typeof input.codex_home === "string" && input.codex_home.trim() ? input.codex_home.trim() : codexHome()
        };
    }
    ...
    // ---------------------------------------------------------------------------
    const { DEFAULT_GLOBAL_CONCURRENCY, defaultConcurrency, normalizeConcurrency, normalizeGlobalConcurrency, createLimiter, emitEvent, log, USAGE_KEYS, emptyUsage, addUsageInto, accountUsage, sumUsageFromWorkers, validateAgainstSchema, stableStringify, stepId } = engineCore;
    ...
    function reserveLaunchStagger(ctx) {
        if (!ctx || !ctx.launchStaggerMs || ctx.concurrency <= 1)
            return 0;
        const now = Date.now();
