...
async function healthyMetadata(metaPath, runsDir) {
    ...
    if (!meta || meta.runs_dir !== runsDir || typeof meta.url !== "string")
        return null;
    ...
    if (typeof meta.static_dir !== "string" || path.resolve(meta.static_dir) !== STATIC_DIR)
        return null;
    try {
...
async function waitForMetadata(metaPath, timeoutMs) {
    ...
    while (Date.now() - startedAt < timeoutMs) {
        ...
        if (last && typeof last.url === "string" && Number.isInteger(last.pid))
            return last;
...
async function startServer(runsDir, metaPath, input = {}) {
    ...
    const env = {
        ...
        ULTRACODE_UI_PORT: input.ui_port === undefined || input.ui_port === null || input.ui_port === "" ? "0" : String(input.ui_port)
    };
...
async function attachWorkflowUi(record, ctx, input = {}) {
    if (!record || !record.state_path || !shouldLaunchUi(input))
        return null;
