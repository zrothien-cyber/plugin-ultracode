...
const { currentProcessIdentity, isLivePid, lookupProcessIdentity, sameCommandLine, sameStartTime } = require("./process-liveness");
function nowIso() {
...
function refreshControllerHeartbeat(record) {
    if (!record || !record.controller || record.status !== "running")
        return record;
...
function markAbandoned(record, reason, observedAt) {
    ...
    if (Array.isArray(next.workers))
        next.workers = next.workers.map(markWorkerAbandoned);
    if (Array.isArray(next.steps) && next.steps !== next.workers)
        next.steps = next.steps.map(markWorkerAbandoned);
    return next;
}
...
function reconcileRunningRecord(record, options = {}) {
    ...
    return {
        record: markAbandoned(record, liveness.reason, observedAt),
        changed: true
    };
}
module.exports = {
