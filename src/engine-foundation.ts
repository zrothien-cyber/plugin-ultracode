...
import type { EngineError, Foundation } from "../scripts/engine-types";
import type { ContextOptions, RunContext } from "./engine-core";

const engineCore: typeof import("./engine-core") = require("./engine-core");
...
const { reconcileRunningRecord } = require("./run-lifecycle") as {
  reconcileRunningRecord(record: unknown): ReconciledRecord;
};
...
function isExecutable(filePath: fsSync.PathLike): boolean {
  try {
...
function normalizeOptions(input: FoundationInput = {}) {
  ...
  if (reasoningEffort !== undefined && !VALID_EFFORTS.has(reasoningEffort as string)) {
...
function selectRoles(count: number) {
...
function planWorkflow(input: FoundationInput = {}) {
...
async function readWorkflow(input: FoundationInput = {}): Promise<unknown> {
  let filePath;
  if (input.state_path) {
...
function createContext(opts: ContextOptions = {}): RunContext {
...
function classifyCodexError(
  ...
  const exec = execResult || (error && (error.codex_exec as CodexExecResult)) || null;
...
function backoffDelay(attempt: number, base: unknown, max: unknown, jitter: unknown): number {
...
function abortableDelay(ms: unknown, signal?: AbortSignal | null): Promise<void> {
...
function abortError(signal?: AbortSignal | null): EngineError {
  const reason = signal && signal.reason !== undefined ? signal.reason : "aborted";
  const err: EngineError = new Error(`cancelled: ${typeof reason === "string" ? reason : "aborted"}`);
  err.code = "ABORT_ERR";
  err.cancelled = true;
  return err;
}

function reserveLaunchStagger(ctx: RunContext | null | undefined): number {
...
async function waitForLaunchStagger(ctx: RunContext | null | undefined, label: unknown, phase: unknown): Promise<void> {
...
function resolveTransport(value: unknown): string {
