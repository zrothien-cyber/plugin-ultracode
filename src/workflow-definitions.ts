...
function workflowSearchDirs(input: RuntimeRecord = {}) {
...
function scopeDir(scope, input: RuntimeRecord = {}) {
  const cwd = path.resolve(input.cwd || process.cwd());
  if (scope === "user") return path.join(input.home || os.homedir(), ".claude", "workflows");
  if (scope === "codex") return path.join(input.codex_home || codexHome(), "ultracode", "workflows");
...
async function listWorkflowDefinitions(input: RuntimeRecord = {}) {
  const found = [];
  const seen = new Set();
...
async function resolveWorkflowDefinition(identifier, input: RuntimeRecord = {}) {
  ...
  if (match) return readDefinitionFile(match.path, match.scope);
  throw new Error(`Workflow "${raw}" was not found in .claude/workflows, ~/.claude/workflows, or CODEX_HOME/ultracode/workflows.`);
}

async function saveWorkflowDefinition(input: RuntimeRecord = {}) {
...
async function updateWorkflowDefinition(identifier, input: RuntimeRecord = {}) {
...
async function deleteWorkflowDefinition(identifier, input: RuntimeRecord = {}) {
  const current = await resolveWorkflowDefinition(identifier, input);
  if (current.scope === "path") {
