...
import type { CodexExecResult, EngineError, Foundation } from "../scripts/engine-types";
import type { EventRecord, RunContext } from "./engine-core";
import type { GlobalLease, GlobalLeaseWait } from "./global-concurrency";

interface AppServerClient {
  _internal: {
    normalizeUsage(breakdown: unknown): unknown;
  };
  runAppServerTurn(input: AppServerTurnInput): Promise<{ execResult: ExecutionResult; value: unknown }>;
}

interface AppServerTurnInput {
  prompt: string;
  schema: unknown;
  opts: {
    bin: string;
...
interface StreamEvent extends EventRecord {
  type?: unknown;
  usage?: unknown;
  thread_id?: unknown;
}

type StreamEventHandler = (event: StreamEvent) => unknown;
type WorkerHookName = "onWorkerPending" | "onWorkerRecord";

interface WorkerPromptInput {
  task: string;
  workflow: { id: unknown; cwd: unknown };
  worker: { title: unknown; focus: unknown };
  sandbox: string;
}
...
interface WorkerOptions {
  ...
  executor?: "cold" | "resume" | "fork";
  transport?: string;
  transport_strict?: unknown;
  transportStrict?: unknown;
  schema?: unknown;
  ...
  retryJitter?: unknown;
  retry_jitter?: unknown;
  label?: string;
  title?: string;
  phase?: unknown;
  isolation?: string;
  script_call_id?: string;
  cache_key?: string;
  ...
  signal?: AbortSignal;
  baseInstructions?: unknown;
  onTransportFallback?: (error: EngineError) => unknown;
}
...
interface ExecutionResult extends CodexExecResult {
  pid: number | null;
...
interface WarmWorkerHandle {
  sessionId: string | null;
  result: WorkerRecord;
  turn(prompt: string, perTurnOpts?: WorkerOptions): Promise<WorkerRecord>;
}

interface WarmStageHelper {
  readonly sessionId: string | null;
  start(prompt: string, perCallOpts?: WorkerOptions): Promise<WorkerRecord>;
  turn(prompt: string, perCallOpts?: WorkerOptions): Promise<WorkerRecord>;
}
...
interface LoopState extends Record<string, unknown> {
  ...
  dryRounds?: number;
  maxRounds?: number;
  lastResult?: WorkerRecord | null;
  lastValue?: unknown;
  lastFresh?: unknown[];
  lastDuplicates?: unknown[];
  round?: number;
  done?: boolean;
}

interface LoopOptions extends WorkerOptions {
  dryRounds?: number;
  maxRounds?: number;
  dedupe?: boolean;
  dedupeFindings?: boolean;
  dedupe_findings?: boolean;
  dedupeSources?: boolean;
  dedupe_sources?: boolean;
  extractItems?: (value: unknown) => unknown[];
  dedupeKey?: (value: unknown) => unknown;
  isDry?: (value: unknown) => boolean;
  seen?: Set<string> | unknown[];
  state?: LoopState;
}

interface VerifyOptions extends WorkerOptions {
  skeptics?: number;
  lenses?: string[];
  describe?: (finding: unknown) => string;
  context?: string;
}
...
function createExecution(foundation: Foundation) {
  const {
    MAX_WORKERS,
    DEFAULT_TIMEOUT_MS,
    VALID_SANDBOXES,
...
function notifyCtxWorkerHook(ctx: RunContext | null, hookName: WorkerHookName, ...args: unknown[]): void {
...
function workerPrompt({ task, workflow, worker, sandbox }: WorkerPromptInput): string {
  return [
...
function buildCodexArgs(opts: ResolvedWorkerOptions, schemaPath: string | null, lastMessagePath: string): string[] {
...
function buildResumeArgs(opts: ResolvedWorkerOptions, sessionId: string, lastMessagePath: string): string[] {
...
function isResumeUnavailable(error: unknown, execResult?: CodexExecResult | null): boolean {
...
function injectSchemaIntoPrompt(prompt: string, schema: unknown): string {
...
function parseUsage(stdout: string): unknown {
...
function spawnCodex({
  ...
  onStreamEvent,
  signal
}: SpawnCodexInput): Promise<ExecutionResult> {
  return new Promise<ExecutionResult>((resolve, reject) => {
    ...
    function handleStdout(text: string): void {
    ...
    function finish(error: EngineError | null, code: number | null, exitSignal: NodeJS.Signals | null): void {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
      if (startupTimer) clearTimeout(startupTimer);
...
function resolveWorkerOpts(opts: WorkerOptions = {}): ResolvedWorkerOptions {
...
function normalizeAppServerUsage(breakdown: unknown): unknown {
  return appServerClient._internal.normalizeUsage(breakdown);
}

async function createWorktree(baseDir: string): Promise<Worktree> {
...
async function collectDiff(worktree: Worktree): Promise<string> {
...
async function runAppServerAttempt({ prompt, schema, opts, onStreamEvent }: AttemptInput): Promise<AttemptResult> {
  const env = {
    ...process.env,
...
}: AttemptInput): Promise<AttemptResult> {
...
function failedWorker(
  label: string,
  phase: unknown,
  error: string,
  codexExec?: CodexExecResult,
  usage?: unknown,
  durationMs?: number,
  status?: string
): WorkerRecord {
...
function cancelledWorker(label: string, phase: unknown, reason?: string): WorkerRecord {
...
async function spawnWorker(prompt: string, opts: WorkerOptions = {}): Promise<WorkerRecord> {
...
async function spawnWorkerGuarded(
  prompt: string,
  opts: ResolvedWorkerOptions,
  ctx: RunContext | null,
  ...
  const workerEvent = (event: EventRecord): EventRecord => ({
    ...event,
  ...
  const capExceeded = (): WorkerRecord | null => {
  ...
  let runOpts: ResolvedWorkerOptions = opts;
    ...
    while (true) {
      ...
      try {
        ...
        globalLease = await acquireGlobalLease({
          ...
          onWait: ({ active, limit }: GlobalLeaseWait) => {
        ...
        attemptResult = await runCodexAttempt({
          ...
          opts: {
            ...
            onTransportFallback: (error: EngineError) => {
...
async function spawnWarmWorker(initialPrompt: string, opts: WorkerOptions = {}): Promise<WarmWorkerHandle> {
  ...
  const executor: "cold" | "fork" | "resume" =
    opts.executor === "cold" || opts.executor === "fork" ? opts.executor : "resume";
  ...
  const handle: WarmWorkerHandle = {
    sessionId,
    result: first,
    async turn(prompt: string, perTurnOpts: WorkerOptions = {}) {
...
function createWarmStageHelper(base: WorkerOptions): WarmStageHelper {
  let handle: WarmWorkerHandle | null = null;
  return {
    get sessionId() {
    ...
    async start(prompt: string, perCallOpts: WorkerOptions = {}) {
    ...
    async turn(prompt: string, perCallOpts: WorkerOptions = {}) {
      if (!handle) {
...
function defaultLoopItems(value: unknown): unknown[] {
...
function stableLoopKey(value: unknown): unknown {
...
async function loopUntilDry(
  makePrompt: (round: number, ctx: RunContext | null, state: LoopState) => string,
  opts: LoopOptions = {}
): Promise<unknown[]> {
  const ctx = opts.ctx || null;
  const schema = opts.schema === undefined ? WORKER_SCHEMA : opts.schema;
  ...
  const isDry =
    typeof opts.isDry === "function"
      ? opts.isDry
      : (result: unknown) => !result || (isRecord(result) && Array.isArray(result.findings) && result.findings.length === 0);
  const collected: unknown[] = [];
  const seen = opts.seen instanceof Set
    ? opts.seen
    : new Set(Array.isArray(opts.seen) ? opts.seen.map((item) => String(item)) : []);
  const state: LoopState = opts.state && typeof opts.state === "object" ? opts.state : {};
  Object.assign(state, {
    collected,
    seen,
...
async function adversarialVerify(findings: unknown[], opts: VerifyOptions = {}): Promise<unknown[]> {
  const ctx = opts.ctx || null;
  ...
  const describe =
    typeof opts.describe === "function"
      ? opts.describe
      : (finding: unknown) => (typeof finding === "string" ? finding : JSON.stringify(finding, null, 2));

  const verdicts = await Promise.all(
    findings.map(async (finding) => {
      const votes = await Promise.all(
        Array.from({ length: skeptics }, (_, i: number) => {
          ...
          return spawnWorker(prompt, {
            ...
            maxDelayMs: firstDefined(opts.maxDelayMs, opts.max_delay_ms),
            retryJitter: firstDefined(opts.retryJitter, opts.retry_jitter)
          }).then((result: WorkerRecord) => (result.status === "completed" ? result.value : null));
        })
      );
