...
function sanitizeMeta(value) {
  ...
  const meta: RuntimeRecord = {};
  for (const key of ["name", "description"]) {
    if (typeof value[key] === "string" && value[key].trim()) meta[key] = value[key].trim();
  }
  if (Array.isArray(value.phases)) {
    meta.phases = Array.from(value.phases)
      .map((phase) => {
        if (typeof phase === "string" && phase.trim()) return { title: phase.trim() };
        const phaseRecord = phase && typeof phase === "object" ? (phase as RuntimeRecord) : null;
        if (phaseRecord && typeof phaseRecord.title === "string" && phaseRecord.title.trim()) {
          const out: RuntimeRecord = { title: phaseRecord.title.trim() };
          for (const key of Object.keys(phaseRecord).sort()) {
            if (key === "title") continue;
            const valueForKey = phaseRecord[key];
            if (typeof valueForKey === "string" && valueForKey.trim()) out[key] = valueForKey.trim();
            else if (typeof valueForKey === "number" || typeof valueForKey === "boolean") out[key] = valueForKey;
          }
          return out;
        }
        return null;
...
function bindingStatementsForImport(text) {
  ...
  if (defaultMatch && !clause.startsWith("{") && !clause.startsWith("*")) {
    ...
    if (!WORKFLOW_BINDINGS.has(local)) statements.push(`const ${local} = orchestrator;`);
  ...
  if (named) {
    for (const binding of named) {
      if (!WORKFLOW_BINDINGS.has(binding.imported)) continue;
      if (!isIdentifier(binding.local) || WORKFLOW_BINDINGS.has(binding.local)) continue;
...
function prepareClaudeWorkflowSource(source) {
  ...
  if (!hasExportedRunFunction(rewritten)) return rewritten;
